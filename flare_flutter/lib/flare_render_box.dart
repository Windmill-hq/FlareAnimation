...
abstract class FlareRenderBox extends RenderBox {
  ...
  final List<FlareCacheJson> _assets = [];
  ...
  void _unload() {
    for (final FlareCacheJson asset in _assets) {
      asset.deref();
    }
    _assets.clear();
    onUnload();
  ...
  Future<FlutterActor> loadFlare(String json) async {
    if (json == null) {
      return null;
    }

    FlareCacheJson asset = await cachedJson(assetBundle, json);
  ...
  /// Load a flare file from cache
  Future<FlutterActor> loadFlareJson(String json) async {
    if (json == null) {
      return null;
    }
